...
- feat: 新增节点支持设置为办理人（仅办理节点）
- feat: 新增支持设置流程实例紧急程度
- opt: 完善注释和说明文档
- opt: 优化节点模型扩展属性提供友好的获取方式
- fixed: 修复实例创建初始化导致更新优先级重置问题

## [v1.2.6] 2026.08.23

- feat: 新增支持无节点条件执行能力
- feat: 升级 mybatis-plus 依赖 3.5.17 版本
- feat: 新增重审已结束的流程实例，恢复实例并将当前任务退回发起人暂存待审
- opt: 优化临时节点减签存在任务情况删除恢复到上一个任务
- opt: 优化支持多分支票签
- opt: 优化定时任务锁新增缓存注释
- opt: 优化支持更复杂的并行分支执行逻辑
- opt: 优化并行分支无任务情况
- opt: 优化并行分支存在任务判断
- opt: 优化完善判断当前执行任务空列表情况
- opt: 优化补充 issues/IJU6OD 测试用例
- fixed: 修复模型获取下一个执行节点复杂情况异常问题
- fixed: 修复主流程启动子流程导致流程实例数据丢失问题

## [v1.2.5] 2026.06.13

- feat: 新增支持根据流程实例ID重新部署当前实例流程模型
- feat: 新增模型节点判断是否为审批提醒节点方法
- feat: 新增历史接口兼容
- opt: 优化允许启动流程自定义模型校验
- opt: 优化完善触发器执行回调逻辑
- opt: 优化抄送人为动态直接获取参与者信息
- opt: 修改动态分配人员类型注释说明
- fixed: 修复并行延迟任务异常
- fixed: 修复会签提前驳回终止异常
- fixed: 修复当实例不存在可能异常问题
- fixed: issues/IJLXAL 修复并行分支多触发器执行问题

## [v1.2.4] 2026.04.12

- feat: 新增 pom 管理版本号
- feat: 新增跳转重新设置处理人测试用例
- opt: 优化重新加载流程模型兼容错误数据
- opt: 优化为草稿主办都为开始节点设定
- opt: 优化支持动态修改设置新的审核人类型
- opt: 优化浏览文件支持多层嵌套子流程关闭
- opt: 优化支持拒绝驳回到子流程内部
- opt: 优化检测模型审批节点配置错误细节
- fixed: 修复解决反复退回拒绝到发起人异常问题
- fixed: 修复删除流程实例未关联子流程问题
- fixed: 修复路由子节点key获取失败问题
- fixed: 修复获取下一个节点不存在子节点情况
- fixed: 修复多个分支没有自己的子节点时报错问题
- fixed: 修复直接退回到发起人节点修改为草稿状态
- fixed: 修复子流程完成重启父流程动态修改模型失败问题
...
- feat: 新增持仓抄送动态扩展提供参与者
- feat: 新增允许删除临时状态节点
- feat: 新增支持临时节点状态
- opt: 优化springboot4版本控制配置
- opt: 优化实现监听异步任务监听处理器注释说明
...
- opt: 任务参与者表task_actor字段extend修改为ext避免SqlParser解析异常
- opt: 优化条件判断 spel 表达式支持 long 类型
- opt: 优化驳回跳转执行对象设置传递新的任务列表
- opt: 优化条件参数获取节点传递
- opt: 优化全局变量追加
